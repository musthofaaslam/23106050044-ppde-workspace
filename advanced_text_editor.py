...
from tkinter import ttk  # Diperlukan untuk ttk.Combobox di change_font
import os
from datetime import datetime
import re # Diperlukan untuk replace_all
...
class AdvancedTextEditor:
    def __init__(self):
        ...
        # Undo/Redo stacks (Not used with scrolledtext, but kept for future expansion)
        self.undo_stack = []
        self.redo_stack = []
        ...
        self.bind_events()
        self.load_settings() # Memuat file yang baru dibuka (jika ada)
        self.update_title()
        self.update_status()

    # --- PRAKTIKUM 2.1: SETUP TEXT EDITOR ---
    def buat_interface(self):
        ...
        # Line numbers frame (optional, implementation detail skipped for simplicity)
        self.line_numbers_frame = tk.Frame(editor_frame, bg="lightgray", width=50)

        # Text area dengan scrollbar
        self.text_area = scrolledtext.ScrolledText(
            editor_frame,
            # Mengatur wrap berdasarkan self.word_wrap
    ...
    # --- HELPER METHOD HILANG ---
    def confirm_save(self):
        """Helper method untuk konfirmasi save sebelum operasi baru"""
        response = messagebox.askyesnocancel(
            "Unsaved Changes",
            "Do you want to save changes to the current file?"
        )
        # Yes = 1, No = 0, Cancel = None
        if response is True:
            return self.save_file() # Coba simpan, kembalikan hasil True/False
        elif response is False:
            return True # Lanjutkan operasi (tidak simpan)
        else:
            return False # Batalkan operasi
        
    def load_settings(self):
        """Placeholder untuk memuat settings (seperti recent files dari file konfigurasi)"""
        # Dalam implementasi nyata, recent_files akan dimuat dari disk
        pass
        ...
        if not self.recent_files:
            ...
            for i, filename in enumerate(self.recent_files):
                ...
                self.recent_menu.add_command(
                    ...
                    # Gunakan lambda untuk melewati argumen ke open_recent_file
    ...
    def cut_text(self):
        ...
        try:
            # Menggunakan event_generate untuk memicu aksi Cut bawaan
    ...
    def copy_text(self):
        ...
        try:
            # Menggunakan event_generate untuk memicu aksi Copy bawaan
    ...
    def paste_text(self):
        ...
        try:
            # Menggunakan event_generate untuk memicu aksi Paste bawaan
    ...
    def select_all(self):
        ...
        # Menambahkan tag SEL (Selection) dari awal "1.0" hingga akhir tk.END
        self.text_area.tag_add(tk.SEL, "1.0", tk.END)
        # Mengatur posisi kursor di awal file
    ...
    def find_next(self):
        ...
        self.text_area.tag_remove("found", "1.0", tk.END) # Hapus highlight find all
        ...
        pos = self.text_area.search(
            ...
            # Whole word logic not fully implemented here for simplicity
        )
        ...
        if pos:
            ...
            # Search from beginning (wrap search)
            pos = self.text_area.search(
                search_text,
                "1.0",
                start_pos,
    ...
    def find_all(self):
        ...
        while True:
            ...
            count += 1
            start_pos = end_pos # Mulai pencarian dari posisi setelah kata yang ditemukan
    ...
    def replace_current(self):
        ...
        try:
            ...
            case_match = (self.replace_case_var.get() and selected_text == find_text)
            nocase_match = (not self.replace_case_var.get() and selected_text.lower() == find_text.lower())
            
            if case_match or nocase_match:
    ...
    def replace_all(self):
        ...
        if self.replace_case_var.get():
            ...
            # Case insensitive replace menggunakan regex
            import re
    ...
    def toggle_line_numbers(self):
        ...
        # Implementation for line numbers would go here (requires complex layout handling)
    ...
    def change_font(self):
        ...
        # Menggunakan font.families() untuk list font yang tersedia, atau list kustom
        font_families = list(font.families()) 
        font_var = tk.StringVar(value=self.current_font[0])
        font_combo = ttk.Combobox(font_dialog, textvariable=font_var, values=font_families, state="readonly")
        font_combo.pack(pady=5)
        font_combo.set(self.current_font[0]) # Set nilai awal
        ...
        def apply_font():
            try:
                ...
                messagebox.showerror("Font Error", f"Failed to apply font: {e}")
            finally:
                font_dialog.destroy()
    ...
    def show_word_count(self):
        ...
        # Hitung kata: split berdasarkan spasi dan filter string kosong
        words = len(content.split()) 
        # Hitung baris: hitung karakter newline
        lines = content.count('\n') + (1 if content else 0)
    ...
    def show_char_count(self):
        ...
        content = self.text_area.get("1.0", tk.END)
        # Karakter termasuk spasi dan newline
        chars = len(content)
        # Karakter tanpa spasi, newline, dan tab
        chars_no_spaces = len(content.replace(' ', '').replace('\n', '').replace('\t', '').strip())

        messagebox.showinfo("Character Count", f"Characters (with spaces/newlines): {chars}\nCharacters (content only): {chars_no_spaces}")
...
Created for educational purposes."""
    ...
    def bind_events(self):
        ...
        # Text change event
        # <<Modified>> event lebih akurat daripada <Key>
        self.text_area.bind('<<Modified>>', self.on_text_change)
        self.text_area.edit_modified(False) # Reset status modified
    ...
    def on_text_change(self, event=None):
        ...
        # Reset flag Modified agar event dipanggil lagi di masa depan
    ...
    def update_cursor_position(self):
        ...
        try:
            ...
            # Kolom di Tkinter dimulai dari 0, tambahkan 1
    ...
    def update_title(self):
        ...
        # Tambahkan (*) jika file dimodifikasi
        title += filename
        if self.is_modified:
            title += " *"
            
        self.window.title(title)
        self.file_info_label.config(text=filename) # Update label file info

    def update_status(self, message="Ready"):
        """Method untuk update pesan di status bar"""
        self.status_label.config(text=message)

    def on_closing(self):
        """Method yang dipanggil saat jendela ditutup"""
        if self.is_modified:
            if not self.confirm_save():
                return
        self.window.destroy()
    ...
    def run(self):
        """Method untuk menjalankan main loop"""
